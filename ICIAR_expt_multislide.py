...
def to_be_reviewed_next_based_on_uncertainity(metadata,indices,prob_preds,prob,q_label):
    to_be_reviewed_next = []
    all_non_match_indices = np.array([i for i in indices if prob_preds[i]!=q_label])
    uncertainity_vector = get_entropy(prob[all_non_match_indices])
    most_uncertain_indices = np.argsort(uncertainity_vector)[::-1]
    reqd_list = np.array(list(metadata['file_path']))[most_uncertain_indices[:20]]
    to_be_reviewed_next.extend(list(reqd_list))    
    return to_be_reviewed_next
...
def to_be_reviewed_next_based_on_uncertainity(metadata,indices,prob_preds,prob,q_label):
    to_be_reviewed_next = []
    _,predicted_classes_array,pred_id = get_ranked_images(search_embeddings,query_embeddings,search_annot,leave_one_out=False,
                                                          n_neighbors=len(search_embeddings),use_gpu_support=True,get_id=True)
    uncertainity_vector = get_entropy(prob[all_non_match_indices])
    most_uncertain_indices = np.argsort(uncertainity_vector)[::-1]
    reqd_list = np.array(list(metadata['file_path']))[most_uncertain_indices[:20]]
    to_be_reviewed_next.extend(list(reqd_list))    
...
def get_pred_results(embeddings_search_original,meta_data_search,reviewed_images_overall,q_label,q_id,trial,csv_init=None,perform_val_only=False,
    ...
    if use_classifier_predictions:
        ...
        meta_data_search_copy['Confidence_0'] = np.array([0]*len(meta_data_search_copy))
        meta_data_search_copy['Confidence_1'] = np.array([0]*len(meta_data_search_copy))
        ...
        meta_data_search_copy['Confidence_0'][list(retrieved_indices.reshape(-1))] = prob_value[:,0].reshape(-1)[retrieved_indices.reshape(-1)]
        meta_data_search_copy['Confidence_1'][list(retrieved_indices.reshape(-1))] = prob_value[:,1].reshape(-1)[retrieved_indices.reshape(-1)]
        meta_data_search_copy['Final_Pred'][reviewed_indices] = meta_data_search_copy['class_name'][reviewed_indices]
        meta_data_search_copy['Confidence_0'][reviewed_indices] = np.array([1]*len(reviewed_indices))
        meta_data_search_copy['Confidence_1'][reviewed_indices] = np.array([1]*len(reviewed_indices))
        ...
        csv_to_be_saved.columns = ['class_id', 'file_path', 'class_name', 'file_mapping','Final_Pred_{}_trial_{}'.format(q_id,trial),'Confidence_0_{}_trial_{}'.format(q_id,trial),'Confidence_1_{}_trial_{}'.format(q_id,trial),'Reviewed_{}_trial_{}'.format(q_id,trial)]
...
def rel_fb_loop(model,classifier,query_idx,embeddings_search_512,meta_data_search,embeddings_query_512,meta_data_query,labels_applicable,rel_images_overall,
    ...
    final_dict['F1_classifier_based_Step_{}'.format(fb_round)] =  [report_search['accuracy']]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--patch_wise_split", type=int, default=0)
    parser.add_argument("--slide_wise_split", type=int, default=0)
    ...
    parser.add_argument("--use_texture_encoder", type=int, default=0)
    ...
    print("Search dir",args.search_dir)
    ...
    use_top_5_pred = 0
    for i in list(meta_data_query.file_path):
